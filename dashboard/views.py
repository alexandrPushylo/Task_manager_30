...
import dashboard.services.construction_site as CONSTR_SITE_SERVICE  #TODO
import dashboard.services.work_day_sheet as WORK_DAY_SERVICE  #TODO
import dashboard.services.driver_sheet as DRIVER_SHEET_SERVICE  #TODO
...
import dashboard.services.application_today as APP_TODAY_SERVICE  #TODO
import dashboard.services.application_technic as APP_TECHNIC_SERVICE  #TODO
import dashboard.services.application_material as APP_MATERIAL_SERVICE  #TODO
...
def dashboard_view(request):
    ...
    if request.method == 'POST':
        ...
        if operation == 'copy':
            target_day_id = request.POST.get('target_day')
            application_today_id = request.POST.get('application_id')
            if Utilities.is_valid_get_request(target_day_id) and Utilities.is_valid_get_request(application_today_id):
                default_app_status = Utilities.get_default_status_for_apps_today(current_user)
                Utilities.copy_application_to_target_day(int(application_today_id), int(target_day_id), default_app_status)
    ...
    # if USERS_SERVICE.is_driver(request.user):
        ...
        if any((
            ...
            if app_today_id:
                ...
                if action == 'restore':
                    ...
                    default_status = U.get_default_status_for_apps_today(current_user)
                    # APP_TODAY_SERVICE.restore_application_today(application_today, default_status)
                    U.restore_application_today(application_today_id=application_today.id, status=default_status)
                else:
                    # APP_TODAY_SERVICE.delete_application_today(application_today)
                    U.delete_application_today(application_today.id)
...
def edit_application_view(request):
    if request.user.is_authenticated:
        ...
        def _create_app_today():
            # _default_status = APP_TODAY_SERVICE.get_default_status_for_apps_today(request.user)
            _default_status = U.get_default_status_for_apps_today(current_user)
            # _application_today = APP_TODAY_SERVICE.create_app_today(
            #     construction_site=construction_site,
            #     date=current_day,
            # )

            create_data = APP_TODAY_SERVICE.CreateApplicationTodaySchema(
                construction_site_id=construction_site.pk,
                date_id=current_day.pk
            )
            _application_today = APP_TODAY_SERVICE.ApplicationTodayService.get_or_create_by_data(create_data)
        ...
        context['is_changeable_material'] = U.Utilities.get_accept_to_change_materials_app(current_workday=current_day)
